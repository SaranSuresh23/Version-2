...
export class EditStandComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private standsDetailsService:StandsDetailsService,private router:Router) { }

  ngOnInit() {
    let productId=this.route.snapshot.paramMap.get('id')
    console.log(productId)
    productId && this.standsDetailsService.getStand(productId).subscribe((data)=>{
      console.log(data)
  ...
  submit(data: any){
    ...
    this.standsDetailsService.editStand(data).subscribe((result)=>{
      console.log(result)
      if(result){
